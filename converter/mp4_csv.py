...
# import pandas as pd
import numpy as np
# Matplotlib is disabled for now, enable it for debugging in anaconda
...
def detectChange(im1, im2, show=False):
    ...
    if np.sum(isDiff) > 20000:
        return 'turn'
    isBlue = (
        np.sqrt(np.sum((im2.astype(float) - [0, 158, 233])**2, axis=2)) < 100)
    changed = isDiff & isBlue
    pim2 = np.lib.pad(im2, ((20, 20), (20, 20), (0, 0)),
...
def match(im, yc, xc, radius, change, show=False):
    ...
    side = max(math.ceil(xc + radius + 2) - col,
               math.ceil(yc + radius + 2) - row)
...
class VideoData(Data):
    ...
    def extractFrames(self):
        ...
        if vc.isOpened():
            ...
            while(rval and c <= self.end - self.init):
                vc.set(cv2.CAP_PROP_POS_FRAMES, c)
                rval1, frame = vc.read()
    ...
    def convertDims(self):
        ...
        PD = [[pdim[0], pdim[2]], [pdim[1], pdim[2]],
              [pdim[0], pdim[3]], [pdim[1], pdim[3]]]
...
class PageData(Data):
    ...
    def toList(self, page, pdim, tdim):
        ...
        for dkey in self.data:
            ...
            count = 0
            if type(temp[0]) == str: 
                ret.append([page + 1, 100 * dkey + count * framelen])
                count += 1
            else: 
                for point in temp:
...
def convert(*args):
   # args should be in this order
    ...
    vid_data = vid.toList()
    print(vid_data)
    headers = ['Page', 'Time', 'X', 'Y', 'Picture Left', 'Picture Right', 'Picture Top', 'Picture Bottom',
               'Text Left', 'Text Right', 'Text Top', 'Text Bottom', 'Gaze Location(0=out, 1=pic, 2=txt)']
    vid_data.insert(0, headers)
    with open(args[0][:-4]+".csv", 'w') as file:
        c = csv.writer(file)
        for x in range(len(vid_data)):
            vid_data[x].insert(0, x)
            c.writerow(vid_data[x])
    print("SAVED")
    return args[0][:-4]+".csv"

    # headers = ['Page', 'Time', 'X', 'Y', 'Picture Left', 'Picture Right', 'Picture Top', 'Picture Bottom', 'Text Left', 'Text Right', 'Text Top', 'Text Bottom', 'Gaze Location(0=out, 1=pic, 2=txt)']
    # vid_data.insert(0, headers)
